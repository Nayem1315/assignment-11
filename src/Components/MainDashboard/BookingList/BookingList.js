...
const BookingList = () => {
  ...
  useEffect(() => {
    fetch(`https://whispering-lowlands-26145.herokuapp.com/getBook/${email}`)
      .then((res) => res.json())
      .then((data) => {
        setBook(data)
        console.log(data);
    ...
    <section >
      <div className="container-fluid row">
        ...
        <div
          ...
          style={{ position: "absolute", left: 320, backgroundColor: "#F4B7C9" }}
        >
          <h2>Service List</h2>
          <Container>
                    ...
                    return serviceDetail.map((pdDetails) => {
                      return (
                        <tr>
                          ...
                          <td className="text-center">
                            {<button className="btn btn-warning">{pd?.status}</button>}
                          </td>
                        </tr>
                      );
                    });
