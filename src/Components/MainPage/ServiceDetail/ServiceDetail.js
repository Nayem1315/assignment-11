...
const ServiceDetail = (props) => {
  ...
  const handleExplore = ((_id, name, price) => {
    ...
    if (loggedInUser) {
      ...
      fetch('https://whispering-lowlands-26145.herokuapp.com/addBook', {
        method: 'POST',
        headers: {
        ...
        <div class="card-body py-3 px-4">
          {
            props.service.image ? <img style={{ height: '100px', width: '100px', borderRadius: "50%" }} src={`data:image/png;base64,${props.service.image.img}`} alt="" />
              :
              <img style={{ height: '100px' }} className="img-fluid mb-3" src={`https://whispering-lowlands-26145.herokuapp.com/${img}`} alt="" />
          }
          <h4 style={{ color: "rgb(212, 12, 79)" }}>{name}</h4>
          <p class="card-text">{description}</p>
          <h5 style={{ color: "rgb(212, 12, 79)" }}>$ {price}</h5>
          <button onClick={() => auth.currentUser?.email ? handleExplore(_id, name, price) : history.push('/login')} className="btn btn-danger">Agree Now</button>
        </div>
      </animated.div>
    </section>
  );
